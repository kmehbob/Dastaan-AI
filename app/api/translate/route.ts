...
export async function POST(request: NextRequest) {
  ...
  if (!apiUrl || !apiKey) {
    return NextResponse.json(
      {
        detail:
          "Private translation service is not configured. Set TRANSLATION_API_URL and TRANSLATION_API_KEY.",
      },
      { status: 503 },
    );
  }
  ...
  try {
    const upstream = await fetch(`${apiUrl.replace(/\/$/, "")}/api/v1/translate`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        text,
        source_language: body.source_language || "ur",
    ...
    return NextResponse.json(
      {
        detail: timedOut
          ...
          : "The private translation service is unavailable.",
      },
